...
검사 항목은 필수 항목 누락, `date`가 오늘(KST)이 아님(테스트는 `--allow-old-date`), 테스트용 옛 비주얼 링크, 결론이 3줄이 아님, 결론 한 줄이 130자(메일에서 약 3줄) 초과, 영상 불릿 하나(결론+본문)가 170자(메일에서 약 4줄) 초과, 불릿 12개 초과, 타임스탬프 표기, 차트 값이 축 범위 밖,
